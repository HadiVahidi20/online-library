 ...
 */
if (!function_exists('online_library_is_plugin_installed')) {
    function online_library_is_plugin_installed($plugin_slug)
    {
        $plugin_path = WP_PLUGIN_DIR . '/' . $plugin_slug;
        return file_exists($plugin_path);
    }
}
if (!function_exists('online_library_is_plugin_activated')) {
    function online_library_is_plugin_activated($plugin_slug)
    {
...
add_action('wp_ajax_online_library_install_and_activate_plugins', 'online_library_install_and_activate_plugins');
add_action('wp_ajax_nopriv_online_library_install_and_activate_plugins', 'online_library_install_and_activate_plugins');
add_action('wp_ajax_online_library_rplugin_activation', 'online_library_rplugin_activation');
add_action('wp_ajax_nopriv_online_library_rplugin_activation', 'online_library_rplugin_activation');
...
function online_library_all_plugins_active() {
...
function online_library_install_and_activate_plugins() {
    if (!current_user_can('manage_options')) {
        return;
    }
    check_ajax_referer('online_library_welcome_nonce', 'nonce');
    ...
    foreach ($recommended_plugins as $plugin) {
        ...
        if (is_online_library_plugin_installed($plugin_slug)) {
...
function is_online_library_plugin_installed($plugin_slug) {
...
add_action('wp_ajax_online_library_dismissed_notice_handler', 'online_library_ajax_notice_dismiss_function');

function online_library_ajax_notice_dismiss_function() {
    if (!wp_verify_nonce($_POST['wpnonce'], 'online_library_welcome_nonce')) {
...
function online_library_custom_admin_notice() {
    if (!get_option('dismissed-get_started_notice', false)) {
        $online_library_current_screen = get_current_screen();
        $online_library_theme = wp_get_theme();
        if ($online_library_current_screen && $online_library_current_screen->id !== 'appearance_page_online-library-guide-page') {
            ?>

            <div class="online-library-admin-notice notice notice-info is-dismissible content-install-plugin theme-info-notice" id="online-library-dismiss-notice" data-notice="get_started_notice">
                <div class="notice-div">
                    <div>
                        <p class="theme-name"><?php echo esc_html($online_library_theme->get('Name')); ?></p>
                        <p><?php _e('For information and detailed instructions, check out our theme documentation.', 'online-library'); ?></p>
                    </div>
                    <div class="notice-buttons-box">
                        ...
                        <a class="button-primary theme-install" id="install-activate-button" href="#"><?php _e('Begin Installation', 'online-library'); ?></a>`
                    </div>
                </div>
            </div>
            <?php
...
add_action('admin_notices', 'online_library_custom_admin_notice');

// After switching theme, reset dismissed notice option
add_action('after_switch_theme', 'online_library_after_switch_theme');
function online_library_after_switch_theme() {
